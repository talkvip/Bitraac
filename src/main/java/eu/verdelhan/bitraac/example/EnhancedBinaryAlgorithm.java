...
public class EnhancedBinaryAlgorithm extends TradingAlgorithm {
    ...
    public Order placeOrder() {
		...
		if (trendCoef > 1.04) {
			// Up trend
			order = new MarketOrder(Order.OrderType.ASK, new BigDecimal(1), Currencies.BTC, Currencies.USD);
		} else if (trendCoef > 1.02) {
			// Up trend
			...
			// Down trend
			order = new MarketOrder(Order.OrderType.BID, new BigDecimal(1), Currencies.BTC, Currencies.USD);
		} else {
			// Stability
			order = null;
		}
